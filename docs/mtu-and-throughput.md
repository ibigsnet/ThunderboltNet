...
- [Overhead: why 1500 hurts at Thunderbolt speeds](#overhead-why-1500-hurts-at-tb-speeds)
...
| Does jumbo unlock dual-lane / much more Gbit/s? | **No.** Line rate is from Thunderbolt training. On 1-lane host-net, lab TCP stayed ~13–14&nbsp;Gbit/s; jumbo mainly cuts retrans/CPU when both ends match. |
| Optional jumbo? | **9000 both ends** if you want fewer packets/s. Higher (16k–64k) possible but less portable. |

---

## Overhead: why 1500 hurts at Thunderbolt speeds
...
| ~20&nbsp;Gbit/s class | Full one way on classic 40G-class Thunderbolt (20 each dir) | ~2.5×10⁹ | ~**1.7 M** | ~**280 k** |
...
On a pure percentage basis jumbo looks like a few percent win. **The larger win is CPU / softirq / syscall overhead** (fewer packets for the same rsync or SMB stream), which is why 10/40/100&nbsp;G Ethernet operators run jumbo on known clean paths — and Thunderbolt host↔host is exactly that kind of path when both ends agree.
...
| Same Thunderbolt line rate | More interrupts, more per-packet work | Far fewer |
...
| 9000 | 9000 | Works; preferred for Thunderbolt bulk |
...
**Windows**: adapter advanced properties / netsh — set the Thunderbolt/USB4 network adapter MTU to the **same** value (9000). Names vary by driver.
...
2. **Desired MTU** → **9000 — recommended for Thunderbolt bulk (both ends)**.  
3. Apply.  
